...
const config = {
	connectionString:
		...
		"postgresql://postgres:32010@localhost:5432/fema_app",
};
...
router.post("/login", async (req, res) => {
	try {
		...
		const token = jwt.sign(
			tokenUser,
			"1a61c95e5a038fef98b630f7e5a01cb2362cac99577787b245cb92ece10af6e"
		);
...
router.post("/registerFacility", async (req, res) => {
	try {
		...
		bcrypt.hash(password, 10).then(async (hashedPass) => {
			await db.none(
				"insert into facilities(facility_name, facility_location, facility_reg, facility_capacity, facility_contacno, facility_email, password) values ($1, $2, $3, $4, $5, $6, $7)",
				[facName, facLocation, facReg, capacity, contactno, email, hashedPass]
			);
			let facilityId = await db.one(
				"select facility_id from facilities where facility_email = $1",
				[email]
			);
			services.forEach((service) =>
				db.none(
					"insert into services(facility_ref, serv_config_ref) values ($1, $2)",
					[facilityId.facility_id, service]
				)
			);
		});
...
router.post("/makebooking", async (req, res) => {
	try {
		...
		// let time = '10:00'
		// let serviceId = 1
		let userRef = await db.oneOrNone(
			"select user_id from users where email = $1",
			[email]
		);
		console.log(userRef);
		if (userRef) {
			let facilityRef = await db.any(
				"select facility_id from facilities where facility_name = $1",
				[facilityName]
			);
			console.log(facilityRef);
			await db.none(
				"insert into bookings(user_ref, facility_ref, service_id, booking_date, booking_time, booking_status) values ($1, $2, $3, $4, $5, $6)",
				[
					userRef.user_id,
					facilityRef[0].facility_id,
					serviceId,
					date,
					time,
					"pending",
...
router.get("/userbookings", async (req, res) => {
	try {
		...
		let userID = await db.any("select user_id from users where email = $1", [
			userEmail,
		]);
		let bookings = await db.any("select * from bookings where user_ref = $1", [
			userID[0].user_id,
		]);
		res.json({
			bookings,
...
router.get("/facilitybookings", async (req, res) => {
	try {
		...
		console.log(facilityRef);
		let bookings = await db.any(
			"select * from bookings where facility_ref = $1",
			[facilityRef[0].facility_id]
		);
		res.json(bookings);
	} catch (error) {
...
router.get("/facilities/:id", async (req, res) => {
	try {
		...
		let facilities = await db.any(
			"select facilities.* from service_config inner join services on serv_config_id = serv_config_ref inner join facilities on facility_id = facility_ref where serv_config_id = $1",
			[serviceId]
		);
		res.json({
			facilities,
...
router.put("/userbookings/:id", async (req, res) => {
	try {
		...
		await db.none(
			"update bookings set booking_date = $1, booking_time = $2 where booking_id = $3",
			[date, time, bookingId]
		);
		await db.none(
			"update bookings set booking_status = $1 where booking_id = $2",
